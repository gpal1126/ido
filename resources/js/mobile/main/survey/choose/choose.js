		...
		$('div[id*=select_img'+(i+1)+']').on({ //답변 이미지 div on
			taphold: function(e) {
				...
				$('.contsDiv'+(i+1)+'').each( function(idx){
					...
					if( $('#surveyForm'+(i+1)+' #select_sub_input_'+(i+1)+'_'+(idx+1)+'').val() == '1' ) { //응답 값이 1이면
						...
						if (ua.indexOf('ipad') > -1){//ipad
							console.log('ios');
							$('#contentsSpan'+(i+1)+'_'+select_id+'').parent().css({'position':'relative', 'top':'-315px'}); //응답 텍스트 위치
						}else {
							$('#contentsSpan'+(i+1)+'_'+select_id+'').parent().css({'position':'relative', 'top':'-153px'}); //응답 텍스트 위치
						}
					} 
				});
				...
				if( $('#chooseImg_'+(i+1)+'_'+id+'').attr('id') == undefined ){
					...
					$('#surveyForm'+(i+1)+' .zoom_btn').css({'top':'-154px'});
				...
				if(zoomBtn_remove == false && $('#surveyForm'+(i+1)+' .choose_img').attr('id') == undefined ){
					...
					if(count <= 1){
						...
						if( $(this).attr('id') ){
							...
							if (ua.indexOf('ipad') > -1){//ipad
								console.log('ios');
								$('#surveyForm'+(i+1)+' .choose_img').css({'top':'-315px'});
								$('#contentsSpan'+(i+1)+'_'+id+'').parent().css({'position':'relative', 'top':'-315px'});
							}else {
								$('#surveyForm'+(i+1)+' .choose_img').css({'top':'-154px'});
								$('#contentsSpan'+(i+1)+'_'+id+'').parent().css({'position':'relative', 'top':'-153px'});
							}
							//$('#contentsSpan'+(i+1)+'_'+id+'').parent().css('top','-154px');
							$(this).css('opacity', '0.6');
							$('#surveyForm'+(i+1)+' .zoom_btn').css('top', '-154px'); //돋보기 이미지 위치 조정
							//선택한 이미지 
			...
			$('#zoomBtn_'+(i+1)+'_'+id+'').on({	
				taphold: function(e) {	//돋보기 제거
					...
					if( $('#surveyForm'+(i+1)+' #select_sub_input_'+(i+1)+'_'+id+'').val() == '1' ) { //응답 값이 1이면
						...
						$('#contentsSpan'+(i+1)+'_'+id+'').parent().css({'position':'relative', 'top':'-153px'}); //응답 텍스트 위치
					} 
