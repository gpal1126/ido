				...
				$(this).each(function(index, p) { //rankWrap이 있는지 없는지 유무로 판단한다.
				    if($('#rankWrap'+id).length > 0){ //rankWrap이 있는지 없는지 유무로 판단한다. (rankWrap이 있을때)
			    		...
			    		if(clickCount < 3){
					    	...
					    	$(this).parent().append('<div id="rankWrap'+id+'" class="rank_img" style="content : url(img/mobile/main/survey/sub_chose_num'+clickCount+'.png);"></div>');
					    	//이미지 opacity 주기
...
//		$('#contsDiv'+(i+1)+'_'+id+' input[name*=rank_sub_input]').parent().after('<div id="rankWrap'+id+'" class="rank_img" style="content : url(img/mobile/main/survey/sub_chose_icon_'+rankValue+'.png);"></div>')
		...
		if(i == 0){
  		...
  		$('.surveyWrap').on('click', '#clearBtn'+(i+1)+'', function(){
  			...
  			$('.contsDiv'+clearId+' div[name=rankCheckbox]').css('opacity', '');	//opacity 제거
  				
  			$('#surveyForm'+clearId+' .rank_img').remove();
  			$('input[name=rank_sub_input'+clearId+']').val(0);
  			console.log(clearId)
  			$('#clearBtn'+clearId+'').remove();
  			$('#nextBtn'+clearId+'').remove();
  			$('#showChart'+clearId+'').remove();
  			
  			//기존 버튼 append 부분 다른 유형과 다름 문제없으면 추후 삭제
  			$('#fBtn'+(i+1)+'').remove();
  			if(i == 0){
  				if((i+1) == $('#total').val()){
  					$('.surveyFooter').append('<div class="next_bt survey_bt finish_bt" id="fBtn'+(i+1)+'" style="width:100%; float:left; ">순위를 매겨주세요</div>');
