				...
				$(this).each(function(index, p) { //rankWrap이 있는지 없는지 유무로 판단한다.
				    if($('#rankWrap_'+(i+1)+'_'+id+'').length > 0){ //rankWrap이 있는지 없는지 유무로 판단한다. (rankWrap이 있을때)
			    		...
			    		if(clickCount < 3){
					    	...
					    	$('#rank_sub_input_'+(i+1)+'_'+id).parent().after('<div id="rankWrap_'+(i+1)+'_'+id+'" class="rank_img" style=""><img class="rank" style="content : url(img/mobile/main/survey/sub_chose_num'+clickCount+'.png); width:110px; margin-top:45px;"></div>');
					    	$('#rankWrap_'+(i+1)+'_'+id+'').next().css({'position':'relative', 'top':'-200px'});
