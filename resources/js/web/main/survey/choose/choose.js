function chooseType(json, i){
		...
		for(j in json.reply){
			...
			$('#surveyForm'+(i+1)+' div[class*="contsDiv"]').css({'width':'220px'});
			...
			switch(json.reply.length){
				case 2 :
  					$('.contsContainer').css({'top':'0'});
  					$('#surveyForm'+(i+1)+' .conts_div').css({'padding-left':'21%'});
  					$('#surveyForm'+(i+1)+' div[class*="contsDiv"]').css({'float':'left'});
  					$('#surveyForm'+(i+1)+' div[class*="contsDiv"]').css({'margin':'15% 0 9% 7%'});  
  					$('#surveyForm'+(i+1)+' div[name$=_s2]').css({'margin-left':'10%'});
			...
			taphold: function(e) {
				...
				$('.contsDiv'+(i+1)+'').each( function(idx){
					...
					if( $('#surveyForm'+(i+1)+' #select_sub_input_'+(i+1)+'_'+(idx+1)+'').val() == '1' ) { //응답 값이 1이면
						...
						$('#contentsSpan'+(i+1)+'_'+select_id+'').parent().css({'position':'relative', 'top':'-220px'}); //응답 텍스트 위치
					} 
				});
				...
				if( $('#chooseImg_'+(i+1)+'_'+id+'').attr('id') == undefined ){
					...
					$('#surveyForm'+(i+1)+' .zoom_btn').css({'top':'-220px'});
					$('#contentsSpan'+(i+1)+'_'+id+'').parent().css({'position':'relative', 'top':'-220px'});
				}//if
				...
				if(zoomBtn_remove == false && $('#surveyForm'+(i+1)+' .choose_img').attr('id') == undefined ){
					...
					if(count <= 1){
						...
						if( $(this).attr('id') ){
							...
							$('#surveyForm'+(i+1)+' .choose_img').css({'top':'-220px'});
							$('#contentsSpan'+(i+1)+'_'+id+'').parent().css({'position':'relative', 'top':'-220px'});
							//$('#contentsSpan'+(i+1)+'_'+id+'').parent().css('top','-154px');
							$(this).css('opacity', '0.6');
							$('#surveyForm'+(i+1)+' .zoom_btn').css('top', '-220px'); //돋보기 이미지 위치 조정
							//선택한 이미지 
			...
			$('#zoomBtn_'+(i+1)+'_'+id+'').on({	
				taphold: function(e) {	//돋보기 제거
					...
					if( $('#surveyForm'+(i+1)+' #select_sub_input_'+(i+1)+'_'+id+'').val() == '1' ) { //응답 값이 1이면
						...
						$('#contentsSpan'+(i+1)+'_'+id+'').parent().css({'position':'relative', 'top':'-220px'}); //응답 텍스트 위치
					} 
		...
		$('.surveyWrap').on('click', '#clearBtn'+(i+1)+'', function(){
			...
			$('#surveyForm'+(i+1)+' .choose_img').remove();	//선택 이미지 제거
			$('input[name=select_sub_input'+(i+1)+']').val(0);	//응답 이미지 값 초기화
			console.log((i+1))
			$('#clearBtn'+(i+1)+'').remove();
			$('#nextBtn'+(i+1)+'').remove();
